...
def news(news_url)
    ...
    for url_no in news_url
        ...
        try
            ...
            # driver.close()
            print(nononono2)
            # driver = webdriver.Chrome(homehadoopProjectpython_testchromedriver)
...
def Url(search)
    ...
    global end_month
    global end_day
    url_temp = 'httpswww.google.comsearchq={search}&tbm=nws'.format(search=search)
    ...
    for pageNo in range(0,300,10)
        ...
        list = soup.select('#search .MjjYuddivdivdiv')
        if not list
            print(no articles)
            break

        for i in list
            if i.select_one('divadivdivnth-child(2)divnth-child(2)')
...
for i in range(10,12)
    ...
    end_day=start_day
    whole_text = 
    textname = homehadoopProjectpython_testdata{search}_.format(search=search.replace( ,_))
